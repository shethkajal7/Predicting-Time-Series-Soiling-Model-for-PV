...
def simulate_PV_time_series(first_date, last_date, freq='1D', degradation_rate=-0.005, 
                            noise_scale=.01, seasonality_scale=.01, nr_of_cleaning_events=120,
                            soiling_rate_low=0.0001, soiling_rate_high=0.003, 
                            smooth_rates=False, random_seed=False):
    ...
    df = pd.DataFrame(index=times, columns=['day', 'noise', 'seasonality', 'degradation', 'soiling',
                                            'cleaning_events'])
    n = len(times)
    ...
    # simulate soiling
    cleaning_events = np.random.choice(df['day'], nr_of_cleaning_events, replace='False')
    ...
    for interval in intervals:
        rate = np.random.uniform(low=soiling_rate_low, high=soiling_rate_high, size=1) * np.ones(len(interval))
        soiling_rate.append(rate)
    df['soiling_rate'] = np.concatenate(soiling_rate)
    if smooth_rates: 
        df.soiling_rate = df.soiling_rate.rolling(smooth_rates,center=True).mean().ffill().bfill()
    derate = np.concatenate([np.cumsum(si) for si in np.split(df['soiling_rate'].values, cleaning_events)])
    df['soiling'] = 1 - derate
...
def simulate_PV_time_series_seasonal_soiling(first_date, last_date, freq='1D', 
                                             degradation_rate=-0.005, noise_scale=.01, 
                                             seasonality_scale=.01, nr_of_cleaning_events=120,
                                             soiling_rate_center=0.002, soiling_rate_std=0.001, 
                                             soiling_seasonality_scale=.9, random_seed=False,
                                             smooth_rates=False, seasonal_rates=False):
    ...
    soiling_seasonality = soiling_rate_center * soiling_seasonality_scale * \
                            np.sin(df['day']/365.25*2*np.pi + np.random.uniform(10, size=1)*np.pi)
    cleaning_probability = ((2-soiling_seasonality_scale)*soiling_seasonality.max()+soiling_seasonality)
    cleaning_probability /= cleaning_probability.sum()
    
    # simulate soiling
    cleaning_events = np.random.choice(df['day'].values, nr_of_cleaning_events, replace='False', 
                                       p=cleaning_probability.values)
    cleaning_events = np.sort(cleaning_events)
    df['cleaning_events'] = [True if day in cleaning_events else False for day in df.day]
    ...
    for interval in intervals:
        ...
        if seasonal_rates:
            if smooth_rates:
                ...
                rate = np.random.normal(soiling_rate_center+soiling_season, soiling_rate_std, 
                                        size=len(interval))
            else:
                soiling_season = soiling_seasonality[pos]
                rate = np.random.normal(soiling_rate_center+soiling_season, soiling_rate_std, 
                                        size=1) * np.ones(len(interval))
        else:
            if smooth_rates: print('Smooth rates not possible without seasonal rates')
            soiling_season = 0
            rate = np.random.normal(soiling_rate_center+soiling_season, soiling_rate_std, size=1) \
...
def generate_my_soiling_signals(high=.02, low=.01, index_form='Datetime'):    
    first_date='2010/01/01'
    last_date='2020/01/01'
    noises = [low, low, high, low, low, low]
    seasonality_scales = [low, high, low, low, low, low]
    soiling_levels = [.003, .001, .001, .0005, .0005, .0001]
    names = ['normal', 'M Soil, H season', 'M Soil, H noise', 'Seasonal cleaning', 
             'L/M Soil (.0005)', 'Low Soil (.0001)']
    dfs = []
    for i in range(6):
        if i==3:
            df = simulate_PV_time_series_seasonal_soiling(first_date, last_date, 
                                                          noise_scale=noises[i], 
                                                          seasonality_scale=seasonality_scales[i],
                                                          soiling_rate_std=0, 
                                                          soiling_rate_center=soiling_levels[i],
                                                          soiling_seasonality_scale=.95,
                                                          smooth_rates=False,
                                                          seasonal_rates=False)
        else:
            df = simulate_PV_time_series(first_date, last_date, 
                                         noise_scale=noises[i], 
                                         seasonality_scale=seasonality_scales[i],
                                         nr_of_cleaning_events=120, 
                                         soiling_rate_low=0, 
                                         soiling_rate_high=soiling_levels[i],
                                         degradation_rate=-.005)
...
if __name__=='__main__':
    ...
    for i in range(N):
        ...
        ax[i%spn].scatter(df.index, df.daily_norm, 5, alpha=.5)
        ax[i%spn].plot(df.index, df.PI_no_noise, 'k', alpha=.5)
